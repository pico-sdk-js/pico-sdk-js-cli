...
export async function readCommand(replServer: PsjReplServer, text: string): Promise<void> {
    ...
    const yargs = Yargs(text)
        .command('* <remote-path>', t('Read a file from the connected device'))
        ...
        .options({
            'local-path': {
                alias: 'p',
                type: 'string',
                description: t('The local file to write from the Pico device'),
                normalize: true
            },
            overwrite: {
                alias: 'o',
                type: 'boolean',
                description: t('Overwrites existing files if they already exist with the same path name'),
                implies: ['local-path']
            }
        })
        .example('.read file.js --local-path ./myFile.js', t('read from the Pico with the file name "file.js" and save the local "myFile.js".'))
        .example('.read file.js', t('read from the Pico with the file name "file.js" and write to teh screen.'))
        .fail((msg: string) => {
            failed = true;
            console.error(msg);
            yargs.showHelp();
    ...
    console.log(t('Reading from "%s"', srcName));
    ...
    if (destName) {
        console.log(t('Writing output to "%s"', destName));
